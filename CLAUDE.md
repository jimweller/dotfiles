...
- The writing contract is split across three files, and `claude_md.md` holds none of it. `configs/claude-code/output-styles/clanker.md` owns the terminal response. `configs/claude-code/rules/ghostwriting.md` owns everything else, including commits, PRs, code comments, docs, Jira, Slack, and email. `configs/claude-code/rules/banned-patterns.md` owns prose mechanics for both. Put a new rule in the file that owns that surface. `claude_md.md` only routes between them. Both rules files carry no `paths` frontmatter, which is what makes them load every session. The style is activated by `"outputStyle": "clanker"` in all three settings files, and the frontmatter `name` must keep matching that value or the style silently resolves to nothing. See `.llmdocs/data-model.md`
