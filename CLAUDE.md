...
- Two CLAUDE.md files exist in this repo: `./CLAUDE.md` is repo-level instructions for the dotfiles project. `configs/claude-code/claude_md.md` is the global agent instruction file, symlinked to both `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` by dotbot. It carries the Daneel persona, the evidence rules, the audience table, general preferences, workflow rules, and the four always-on writing sections (Banned Patterns, Chat Register, Ghostwriting, LSP-First Navigation). One body, both harnesses. Keep it harness-neutral: no Claude-only tool names, no Codex-only config keys.
