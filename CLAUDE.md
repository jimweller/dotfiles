...
- Two CLAUDE.md files exist in this repo: `./CLAUDE.md` is repo-level instructions for the dotfiles project. `configs/claude-code/claude_md.md` is the global agent instruction file, symlinked to both `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` by dotbot. It carries the Daneel persona, the evidence rules, the audience table, general preferences, workflow rules, and the four always-on writing sections (Banned Patterns, Chat Register, Ghostwriting, LSP-First Navigation). One body, both harnesses. Keep it harness-neutral: no Claude-only tool names, no Codex-only config keys. A `<prose-contract>` tag wraps `How to Write`, `Banned Patterns in All Writing`, and `Ghostwriting for Other Humans`, which is why those three are contiguous and `Chat Register` sits below them rather than between. Every rule inside the tag opens with a `` `PC-` `` id. The `prose` skill and the eval suite address rules by that id, so renaming one breaks both. `submodules/clanker-skills/skills/universal/prose/evals/tools/check-anchors.py` fails when the contract and the case files disagree.
- The writing contract is split across the built-in `Concise` output style and `claude_md.md`. `Concise` ships in the Claude Code binary and owns brevity in the terminal response; it is activated by `"outputStyle": "Concise"` in all three settings files, and the capital C matters because the value is looked up against a map keyed by style name. Codex has no `Concise` analogue, so it gets ordering and register but not brevity. Everything else lives in `claude_md.md`: the `## Chat Register` section owns response ordering and the CLANKER voice, `## Ghostwriting for Other Humans` owns commits, PRs, code comments, docs, Jira, Slack, and email, and `## Banned Patterns in All Writing` owns prose mechanics for both surfaces. Put a new rule in the section that owns that surface. `configs/claude-code/rules/` holds only the nine `paths`-filtered language rules, which Claude loads on a matching file Read and Codex never sees. See `.llmdocs/data-model.md`
