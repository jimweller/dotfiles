...
- Two CLAUDE.md files exist in this repo: `./CLAUDE.md` is repo-level instructions for the dotfiles project. `configs/claude-code/claude_md.md` is the global agent instruction file, symlinked to both `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` by dotbot. It carries the Daneel persona, the evidence rules, general preferences, workflow rules, STARTER_CHARACTER, and LSP-First Navigation. One body, both harnesses. Keep it harness-neutral: no Claude-only tool names, no Codex-only config keys. The audience table, the `<prose-contract>` block (`How to Write`, `Banned Patterns in All Writing`, `Ghostwriting for Other Humans`), and the `## Clanker Register` section moved out of this file into two Claude-only plugins (see below); Codex now carries neither. `claude_md.md` keeps a one-line backstop in `## General Preferences` telling the agent to check for and load a prose-contract or writing-rules reference before writing prose, worded with no tool name so it stays harness-neutral.
- The writing contract is split across the built-in `Concise` output style and two plugins. `Concise` ships in the Claude Code binary and owns brevity in the terminal response; it is activated by `"outputStyle": "Concise"` in all three settings files, and the capital C matters because the value is looked up against a map keyed by style name. Codex has no `Concise` analogue, so it gets ordering and register but not brevity. `submodules/clanker-chat-plugin/rules/clanker-register.md` owns response ordering and the CLANKER voice (`<clanker-register>`, `CR-` ids), delivered by a `SessionStart` hook pointer plus a per-turn `UserPromptSubmit` reminder, and paired with the `Clanker` output style. `submodules/clanker-prose-plugin/skills/prose/SKILL.md` owns `Ghostwriting for Other Humans` and `Banned Patterns in All Writing` (`<prose-contract>`, `PC-` ids), delivered by a `SessionStart`/`SubagentStart` hook pointer and doubling as the `prose` skill. Neither plugin prints its rules file directly through a hook's `additionalContext`: that channel truncates around 2 KB, so each hook instead prints a short pointer and the agent follows it with `Read` or `Skill`, which is uncapped. `submodules/clanker-prose-plugin/evals/tools/check-anchors.py` fails when the contract and the case files disagree. `configs/claude-code/rules/` holds only the nine `paths`-filtered language rules, which Claude loads on a matching file Read and Codex never sees. See `.llmdocs/data-model.md`
