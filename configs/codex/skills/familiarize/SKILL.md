...
### 3a. Instruction files

Find all `AGENTS.md` and `CLAUDE.md` files under `SCAN_ROOT` (exclude `.git/` paths). Read each one.

```bash
find "$SCAN_ROOT" \( -name "AGENTS.md" -o -name "CLAUDE.md" \) -not -path "*/.git/*" -not -path "*/node_modules/*"
```
