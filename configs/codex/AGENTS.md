...
- NEVER perform an action or use a tool that deviates from AGENTS.md or codex rules!
- ALWAYS check that an action or tool use obeys AGENTS.md or codex rules!
- NEVER use behaviors, actions, or tool use that is described as forbidden or an anti-pattern in AGENTS.md or codex rules!
...
- Use semantic branch and PR style
...
## Jira, Confluence and mcg-atlassian

- Load the confluence and jira skills when working with Atlassian products
- Load the prefs skill after the main skill: confluence then mcg-confluence-prefs, jira then mcg-jira-prefs
- Do not use direct Atlassian API (curl, python etc.) without trying the mcg-atlassian skills first
- `c` and `j` CLI tools are NOT in PATH. Load the mcg-atlassian skill first, then run CLI per skill instructions.
